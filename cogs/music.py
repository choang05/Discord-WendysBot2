...
from cogs.utils.playlist import Playlist
from cogs.utils.player import MusicPlayer
from cogs.utils.config import Config, ConfigDefaults
from cogs.utils.permissions import Permissions, PermissionsDefaults
from cogs.utils.utils import load_file, write_file, sane_round_int

from cogs.utils import exceptions
from cogs.utils import downloader
from cogs.utils.opus_loader import load_opus_lib
from cogs.utils.constants import VERSION as BOTVERSION
from cogs.utils.constants import DISCORD_MSG_CHAR_LIMIT, AUDIO_CACHE_PATH
...
class Music(discord.Client):
    def __init__(self, bot, config_file=ConfigDefaults.options_file, perms_file=PermissionsDefaults.perms_file):
        self.bot = bot
        self.toggle = False
    ...
    async def cmd_help(self, command=None):
        ...
        if command:
            ...
            if cmd:
                ...
                return Response("No such command", delete_after=0)
            ...
            for attribute in dir(self):
                if attribute.startswith('cmd_') and attribute != 'cmd_help':
            ...
            return Response(helpmsg, reply=True, delete_after=0)
    '''
    '''
    ...
    async def cmd_play(self, player, channel, author, permissions, leftover_args, song_url):
        ...
        if info.get('url', '').startswith('ytsearch'):
            ...
            if not info:
                raise exceptions.CommandError(
                    "Error extracting info from search string, YouTube returned no data.  "
    ...
    async def cmd_clearsongs(self, player, author):
        """
        Usage:
            {command_prefix}clearsongs
    ...
    async def cmd_remove(self, player, leftover_args, index):
        ...
        try:
            ...
            reply_text = "Specify a playlist index to remove!"
    ...
    #  CHAD Wendys
    # async def cmd_setplayingstatus(self, author, status):
    #     """
    #          Usage:
    #              {command_prefix}setplayingstatus [status]
    #
    #          Sets your playing status given a status message.
    #      """
    #     #client = discord.Client
    #     #await self.change_presence(game=discord.Game(name='my game'))
    #     print(self)
    #     await print(self.get_user_info('137062668149325824'))
    #     #await message.author.ws.change_presence(game=discord.Game(name=status))
    #     #await discord.Client.change_status('Hello world')
    #
    #     #message.author.change_presence(game=discord.Game(name=status))
    #     #return Response(self.response, reply=True)

    # async def cmd_move(self, channel, player, currentIndex, newIndex, leftover_args):
    #     """
    #          Usage:
    #              {command_prefix}move [index of song] [index where song will be moved to]
    #
    #          Moves a song in the queue given it's index and the index it will be moved to.
    #          """
    #
    #     try:
    #         currentPos = int(' '.join([currentIndex, *leftover_args]))
    #         playlist_size = len(player.playlist.entries)
    #         if currentPos > playlist_size:
    #             if (playlist_size > 1):
    #                 reply_text = "There are only %s songs in the queue!"
    #                 reply_text %= (playlist_size)
    #                 return Response(reply_text, delete_after=30)
    ...
    #                 return Response(reply_text, delete_after=30)
    #
    #         newPosition = int(' '.join([newIndex, *leftover_args]))
    #         playlist_size = len(player.playlist.entries)
    #         if newPosition > playlist_size:
    #             if (playlist_size > 1):
    #                 reply_text = "There are only %s songs in the queue!"
    #                 reply_text %= (playlist_size)
    #                 return Response(reply_text, delete_after=30)
    ...
    #                 return Response(reply_text, delete_after=30)
    #
    #         entry = await player.playlist.move_entry(currentPos, newPosition)
    #         reply_text = "Moved **%s** from position {} to position {} in the playlist".format(currentIndex, newIndex)
    #         reply_text %= (entry.title)
    #
    #         return Response(reply_text, delete_after=30)
    #     except ValueError:
    ...
    async def on_message(self, message):
        ...
        # Display error when attemping to message bot privately
